...
// Load signing credentials from keystore.properties (git-ignored, keep out of VCS).
...
android {
    compileSdk = 36
    namespace = "com.dxcool.museaudio"

    signingConfigs {
        create("release") {
            if (keystoreProperties.containsKey("storeFile")) {
                val raw = keystoreProperties.getProperty("storeFile", "")
                storeFile = if (File(raw).isAbsolute) File(raw) else project.file(raw)
    ...
    defaultConfig {
        ...
        applicationId = "com.dxcool.museaudio"
        minSdk = 24
        targetSdk = 36
